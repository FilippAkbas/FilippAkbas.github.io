...
/*NOT : 324 line CHANGE RADUİS        1847 line     CHANGE POSİTİON TO BLACK ROUND */
...
(function (S) {
    ...
    function VirtualSky(input){
        ...
        // Projections
        // Change polar radius at h/2 to h/2.4
        ...
        this.colours = {
            ...
            'negative':{
                ...
                'white':"rgb(0,0,0)", //rgb(229,232,235)  -- rgb(235,237,238)
                'grey':"rgb(100,100,100)",
                'stars':'rgb(0,0,0)',
                'sun':'rgb(0,0,0)',
                'moon':'rgb(0,0,0)',
        ...
        this.langs = {
            ...
            'tr': { "language": {"name": "Turkey","alignment": "left" } },
    ...
    VirtualSky.prototype.createSky = function(){
        ...
        this.container.html('<canvas id="'+this.idinner+'" style="display:block;overflow: hidden;"></canvas>');
            ...
            if(this.container){
                ...
                elem.on('mouseover',{me:this},function(e){e.data.me.mouseover = true;})
                    .css({
                        ...
                        'color':(this.negative ? this.col.black : this.col.black),
                        'opacity':1
                    });
    ...
    VirtualSky.prototype.draw = function(proj){
        ...
        c.fillStyle = (this.polartype || this.fullsky) ? this.background : ((this.negative) ? black : black);
        c.fillRect(0,0,this.wide,this.tall);
        c.fill();
    

        let blackRadius = 2.35;
        let blackRadiusTopPos = 2.30;

        if ($(window).width() < 768) {
            blackRadius = 2.25;
            blackRadiusTopPos = 2.25;
        }

        if ($(window).width() > 1000 && $(window).width() < 1400 ) {
            blackRadius = 2.1;
            blackRadiusTopPos = 2.1;
        }
        ...
        c.lineWidth = 1.5;
        this.setFont();
        this.container.css({'font-size':this.fontsize()+'px','position':'absolute'});
    ...
    VirtualSky.prototype.addPointer = function(input){
        ...
        if(matched < 0){
            ...
            //$('#demo-star').html('<div class="virtualsky_infocredit">' + '<a href="'+url+'" style="color: white;">'+credit+'</a>'+ '</div>'+ '<a href="'+url+'" style="display:block;'+style+'">'+'<img src="'+img+'" style="border:0px;'+style+'" title="'+label+'" />'+'</a>');
            $('#demo-star').html('demo');
            alert('true');
            this.pointers[i] = p;
        }
