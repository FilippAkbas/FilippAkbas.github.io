<template>
  <SessionChartWrapper>
    <sdCards v-if="deviceState" title="Cihaza Göre Oturum Sayısı">
      <template #button>
        <div class="card-nav">
          <ul>
      ...
      <div v-else class="session-chart-inner">
        <div :style="{ position: 'relative' }">
          ...
          <Chart
            class="device"
            type="doughnut"
            :labels="['Bilgisayar', 'Mobil', 'Tablet']"
            :datasets="[
              {
                data: deviceState,
                backgroundColor: ['#20C997', '#5F63F2', '#FA8B0C'],
...
const SessionsByDeviceOne = {
  name: 'SessionsByDeviceOne',
...
export default SessionsByDeviceOne;
</script>
